		...
		public static void Parse(string message)
		{
			...
			if (primaryQueryResult != null)
			{
				...
				foreach (var x in primaryQuery)
				{
					...
					else if (x.CommandParameter != null)
					{
						//ensures a parameter is given
						if (userParameter == "") Sendmessage("A word or name must be provided.");

						//setting a variable in ModVariables
						else
							switch (x.Action)
							{
								case "set":
			...
			if (!message.StartsWith("asynctest")) return; //http://stackoverflow.com/questions/100841/artificially-create-a-connection-timeout-error
